...
import * as cmApi from '../cost-model/api.js?v=20260728-s7a';
...
function updateRunButtonState() {
  ...
  btn.setAttribute('title', isClean
    ? 'Inputs unchanged since the last solve — k-means centers match the current points + parameters. Click to force a re-run.'
...
async function renderLanding() {
  ...
  await renderScenarioLanding(rootEl, {
    ...
    getId: (r) => r.id,
    getName: (r) => r.name || r.scenario_data?.name || 'Untitled COG scenario',
            ...
            <table class="u-table">
              ...
              <tbody>
                ${(() => {
                  ...
                  const badgeBg = exc ? 'var(--c-danger-bg)'
                    : p.type === 'demand' ? 'var(--c-info-bg)'
                    : p.type === 'supply' ? 'var(--c-success-bg)' : 'var(--c-warn-bg)';
                  const badgeFg = exc ? 'var(--c-danger-strong)'
                    : p.type === 'demand' ? 'var(--c-info-strong)'
                    : p.type === 'supply' ? 'var(--c-success-strong)' : 'var(--c-warn-ink)';
  ...
  el.innerHTML = `
    <div>
      ...
      ${cogResult && recK != null ? (() => {
        ...
        const accent = matches ? 'var(--c-success-bright)' : 'var(--c-warn)';
        const accentDark = matches ? '#059669' : 'var(--c-warn-deep)';
        const bgGrad = matches ? 'linear-gradient(135deg,var(--c-success-soft),#f0f9ff)' : 'linear-gradient(135deg,var(--c-warn-soft),var(--c-warn-bg))';
        return `
  ...
  if (!hasAssignments) {
    el.innerHTML = `
      <div class="hub-card" style="max-width:900px;border-left:3px solid var(--ies-orange);">
        ...
        <p class="text-body">This scenario has summary results but lacks the per-point assignments needed for the full analysis view. Click the <strong>Run</strong> button in the toolbar above to rebuild the full solve from the current points + parameters.</p>
  ...
  el.innerHTML = `
    <div>
      ${(() => {
        ...
        const goodColor = 'var(--c-success-strong)';  // green = savings / improvement
        const badColor = 'var(--c-danger-strong)';    // red = worse
        const sign = (n) => n > 0 ? '+' : '';
          ...
          <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:16px;font-size:13px;">
            ...
            <div>
              ...
              <div class="u-semibold">${c.totalWeight.toLocaleString()}${(() => {
                ...
                const color = over ? 'var(--c-danger-strong)' : (u > 90 ? 'var(--c-warn-strong)' : 'var(--ies-gray-400)');
      ...
      ` : `
        <div class="hub-card" style="padding:16px 18px;">
          ...
          <div style="font-size:11px;color:var(--ies-gray-400);margin-top:14px;line-height:1.5;border-top:1px dashed var(--ies-gray-200);padding-top:10px;">
            Compared columns are re-computed from each scenario's saved points + parameters on the current engine, so every column shares one cost basis. A column marked <strong>est</strong> couldn't be re-run (its saved row lacks demand points) and shows its saved result as-is — open and re-save that scenario to upgrade it. '—' means the metric isn't available for that scenario.
          </div>
        </div>
      `}
    </div>
...
// EXPORT / INTEGRATION
// ============================================================
// C3 split (2026-07-22): generation logic lives in ./export.js (pure,
// side-effect-free at import — node-testable). These wrappers stay the
// thin triggers: guard toasts, downloads, popup plumbing, CDN loader.

/**
 * Export current analysis to CSV.
 * F1 (P0) — CSV Export. Generation: buildAnalysisCsv (export.js).
 */
function exportCogAnalysis() {
  ...
  const { filename, csv } = buildAnalysisCsv({ cogResult, points, config });
  downloadCSV(csv, filename);
  showToast('Analysis exported successfully', 'success');
}

/**
 * 2026-05-28 36 — Per-shipment cost CSV, auditable trail for contract
 * negotiation. Generation: buildPerShipmentCsv (export.js).
 */
function exportCogPerShipment() {
  ...
  const { filename, csv } = buildPerShipmentCsv({ cogResult, points, config, scenarioName: _scenarioName });
  downloadCSV(csv, filename);
 ...
 * Generation: buildGeoJson (export.js).
 */
function exportCogGeoJSON() {
  ...
  const { filename, text } = buildGeoJson({ cogResult, points, config });
...
/**
 * F3 — PowerPoint deck export trigger. Lazy-loads PptxGenJS from CDN on
 * first use so initial page load isn't impacted; deck structure lives in
 * buildPptxDeck (export.js).
 */
...
async function openPptxExport() {
  ...
  try {
    ...
    const { pres, fname } = buildPptxDeck({
      PptxGenJSCtor, cogResult, points,
      solvePoints: _pointsForSolve(), sensitivityData,
      config, scenarioName: _scenarioName,
    });
 ...
 * Document build: buildPrintHtml (export.js).
 */
function openPrintView() {
  ...
  const html = buildPrintHtml({
    cogResult, points, solvePoints: _pointsForSolve(),
    config, scenarioName: _scenarioName, cpm: _resolveCpm(),
  });
  win.document.open();
