...
import {
  ...
  OFP_MHE_ICONS as _OFP_MHE_ICONS,
  OFP_IT_ICONS as _OFP_IT_ICONS,
  OFP_UOMS as _OFP_UOMS,
  ofpMheIconKey as _ofpMheIconKey,
  ofpItIconKey as _ofpItIconKey,
  ofpEquipLabel as _ofpEquipLabel,
...
import {
  ...
  ofpEnsureFlowRegistry as _ofpEnsureFlowRegistry,
  ofpFlowRegistry as _ofpFlowRegistry,
  ofpFlowMeta as _ofpFlowMeta,
  ofpFlowLabel as _ofpFlowLabel,
  ofpRegisterFlow as _ofpRegisterFlow,
  ofpNormalizeAreaSortOrder as _ofpNormalizeAreaSortOrder,
  ofpNormalizeFlowSortOrder as _ofpNormalizeFlowSortOrder,
  ofpNormalizeSubAreaSortOrder as _ofpNormalizeSubAreaSortOrder,
  ofpReorderArea as _ofpReorderArea,
  ofpReorderFlow as _ofpReorderFlow,
  ofpMoveSubAreaUp as _ofpMoveSubAreaUp,
  ofpMoveSubAreaDown as _ofpMoveSubAreaDown,
  ofpMoveAreaUp as _ofpMoveAreaUp,
  ofpMoveAreaDown as _ofpMoveAreaDown,
  ofpMoveFlowUp as _ofpMoveFlowUp,
  ofpMoveFlowDown as _ofpMoveFlowDown,
  ofpAllFlowTags as _ofpAllFlowTags,
...
import {
  ...
  renderOfpArea as _renderOfpArea,
  renderOfpNode as _renderOfpNode,
    ...
    if (pendingOpen) {
      ...
      if (payload && payload.at && (Date.now() - payload.at) < 60000 && payload.id) {
        ...
        if (full) {
          ...
          // activate the channel + Volumes section even though the
          // pending-open hand-off worked.
          try {
            const dHint = consumeCmDrillbackHint();
            if (dHint && String(dHint.cmId) === String(model.id) && dHint.channelKey) {
              const ch = (model.channels || []).find(c => c.key === dHint.channelKey);
              if (ch) {
                _activeChannelKey = ch.key;
                activeSection = 'volumes';
              }
            }
          } catch (err) { console.warn('[CM] drillback hint consume (mount) failed:', err); }
        }
      } else {
        sessionStorage.removeItem('cm_pending_open');
      }
    ...
    if (pendingNew) {
      ...
      if (payload && payload.at && (Date.now() - payload.at) < 60000 && payload.dealId) {
        ...
        if (model && model.projectDetails) {
          ...
          // S1 (2026-07-22): "New scenario" launched from a Site page
          // pre-stamps the site so the save lands attached, not Unassigned.
          if (payload.siteId) model.projectDetails.siteId = payload.siteId;
          _markCmDirty();
        }
      } else {
        sessionStorage.removeItem('cm_pending_new_for_deal');
...
async function loadModelByCmId(id) {
  ...
  try {
    ...
    if (full.project_data) {
      ...
      // S1 (2026-07-22): hydrate the site binding so save round-trips keep it.
      if (!pdHydrate.siteId && full.site_id) pdHydrate.siteId = full.site_id;
...
// Phase 2a (2026-06-10): buildMetricsOpts — discount/reinvest now read
  ...
  return `
    ...
    ${renderSensitivityCard({ laborLines: model?.laborLines || [], lastCalcHeuristics: _lastCalcHeuristics, marketLaborProfile: currentMarketLaborProfile, scenarios })}
...
function createEmptyModel() {
  ...
  return {
    ...
    // Used by renderImplementation({ model, calc, createEmptyModel, escapeAttr, escapeHtml, renderImplPhaseStepperContent, renderImplRampPanel }) and (in future) by the monthly engine
